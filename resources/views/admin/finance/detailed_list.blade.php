...
<body layadmin-themealias="green" style="">
  ...
  <div class="layui-fluid" id="LAY-app-message">
    <div class="layui-card">
         ...
         <!--         <button class="layui-btn" data-type="reload" id="exceldc">导出数据</button> -->
            </div>
	...
	<script>
		layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element'], function(){
	       ...
	       window.exportexcel =  function(){
				 location.href='/admin/exportexcel?acc='+$("#acc").val()+"&start="+$("#date").val()+"&end="+$("#date1").val();
			}
	       
			window.initsjx =  function(){
				table.render({
					elem: '#sjxtablelist'
					,url:'/admin/get_detailed_list?acc='+$("#acc").val()+"&start="+$("#date").val()+"&end="+$("#date1").val()
					,cols: [[
						 {field:'username', title: '会员编号'}
						 ,{field:'created_at',title: '时间'}
						 ,{field:'change',title: '改变量'}
						 ,{field:'balance',title: '改变前余额'}
						 ,{field:'money_type',title: '状态'}
						 ,{field:'type',title: '改变原因'}
					]]
					,page: true
				});
			}
