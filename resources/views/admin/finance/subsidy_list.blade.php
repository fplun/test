		...
		layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element'], function(){
	       ...
	       window.closeadd = function(){
			...
			window.initsjx =  function(){
				table.render({
					elem: '#sjxtablelist'
					,url:'/admin/get_subsidy_list?acc='+$("#acc").val()+"&start="+$("#date").val()+"&end="+$("#date1").val()
					,cols: [[
						 {field:'username', title: '会员编号'}
						 ,{field:'name',title: '会员姓名'}
						 ,{field:'money',title: '收益数量'}
						 ,{field:'state',title: '是否转换'}
						 ,{field:'type',title: '收益类型'}
						,{field:'created_at',title: '结算日期'}
					]]
					,page: true
				});
			}
